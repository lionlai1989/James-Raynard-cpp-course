/**
 * Lock-free Queue
 * In this example, we go through many troubles to implement a lock-free queue.
 *
 * However, the implementation is broken and has data race:
 *  - iHead and iTail can be accessed from different threads.
 *  - At least one thread modifies them.
 *  - The threads are not synchronized when accessing iHead and iTail.
 *
 * iHead and iTail cannot be atomic types because `std::list<T>::iterator` is not trivially
 * copyable. In the end, we must use mutexes to protect the access to the iHead, iTail and
 * std::list.
 *
 * Lock-free programming is very DIFFICULT to implement correctly.
 */
...
// g++ -std=c++20 -Wall -Wextra -pedantic -pthread 061-lock_free_programming_lock_free_queue.cpp &&
// ./a.out
int main() {
    ...
    for (int i = 0; i < 10; ++i) {
         ...
         *  - Second argument is the address of the object the member function is called on (this
         *    will be the "this" pointer in the member function)
         *  - Then the arguments to the member function. These are passed as reference to int so we
         *    need to wrap them in std::ref()
         */
